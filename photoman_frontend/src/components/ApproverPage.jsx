...
export default function ApproverPage({ user, setUser }) {
  ...
  // Load users on mount
  useEffect(() => {
    fetch("http://150.230.132.224:8000/get_users")
      .then((res) => res.json())
  ...
  const handleUserClick = (userItem) => {
    ...
    setFolders([]);
    setPhotos([]);
    fetch(`http://150.230.132.224:8000/get_folders?user_id=${userItem.id}`)
      .then((res) => res.json())
  ...
  const handleFolderClick = (folder) => {
    ...
    setPhotos([]);
    fetch(
      `http://150.230.132.224:8000/get_photos?user_id=${selectedUser.id}&folder=${folder}`
    )
      .then((res) => res.json())
  ...
  const handleApprove = async () => {
    ...
    setLoading(true);
    await fetch("http://150.230.132.224:8000/approve_photos", {
      method: "POST",
      body: formData,
    });
